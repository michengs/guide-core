...
module.exports.callback = (deps, guide, event) => {
	...
	const result = /\d+/g.exec(event.message);
	if (result && source_ent)
		return guide.handleEvent(["qb", source_ent.huntingZoneId, source_ent.templateId, parseInt(result[0])], source_ent, module.exports.debug);
};