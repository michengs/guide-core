...
class Guide extends EventEmitter {
	...
	async init() {
		...
		let guideFiles = await readdir(path.resolve(this.__mod.info.path, "guides_TW"));			
		...
		try {
			...
			guideFiles.forEach(file => {
				...
				let lineReader = readline.createInterface({
					"input": fs.createReadStream(path.resolve(this.__mod.info.path, "guides_TW", file))
				});
	...
	load(zoneId = false, debug_enabled = false) {
		...
		try {			
			if (this.__lang.strings.region == "TW"){
			 object = require(path.resolve(this.__mod.info.path, "guides_TW", this.obj.id.toString()));
		    } else if (this.__lang.strings.region == "RU"){
			 object = require(path.resolve(this.__mod.info.path, "guides_RU", this.obj.id.toString()));				
			} else if (this.__lang.strings.region == "KR"){
			 object = require(path.resolve(this.__mod.info.path, "guides_KR", this.obj.id.toString()));				
			} else if (this.__lang.strings.region == "JP"){
			 object = require(path.resolve(this.__mod.info.path, "guides_JP", this.obj.id.toString()));				
			} else if (this.__lang.strings.region == "FR"){
			 object = require(path.resolve(this.__mod.info.path, "guides_FR", this.obj.id.toString()));				
			} else if (this.__lang.strings.region == "DE"){
			 object = require(path.resolve(this.__mod.info.path, "guides_DE", this.obj.id.toString()));				
			} else if (this.__lang.strings.region == "ES"){
			 object = require(path.resolve(this.__mod.info.path, "guides_ES", this.obj.id.toString()));				
			} else if (this.__lang.strings.region == "PT"){
			 object = require(path.resolve(this.__mod.info.path, "guides_PT", this.obj.id.toString()));				
			}else {
			 object = require(path.resolve(this.__mod.info.path, "guides", this.obj.id.toString()));				
			}
			// Remove cached guide from require cache
			try {								
			if (this.__lang.strings.region == "TW"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_TW", this.obj.id.toString()))];
		    } else if (this.__lang.strings.region == "RU"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_RU", this.obj.id.toString()))];			
			} else if (this.__lang.strings.region == "KR"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_KR", this.obj.id.toString()))];			
			} else if (this.__lang.strings.region == "JP"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_JP", this.obj.id.toString()))];			
			} else if (this.__lang.strings.region == "FR"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_FR", this.obj.id.toString()))];			
			} else if (this.__lang.strings.region == "DE"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_DE", this.obj.id.toString()))];			
			} else if (this.__lang.strings.region == "ES"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_ES", this.obj.id.toString()))];				
			} else if (this.__lang.strings.region == "PT"){
			  delete require.cache[require.resolve(path.resolve(this.__mod.info.path, "guides_PT", this.obj.id.toString()))];				
			} else {
